	...
	function moveAble(dom: HTMLDivElement){
		...
		window.addEventListener("mousemove", (e:MouseEvent) => {
			...
			if(parseInt(getComputedStyle(dom).bottom) < 0 ){
				dom.style.top = (dom.parentElement as HTMLDivElement).clientHeight - dom.offsetHeight + "px"
			}
			if(parseInt(getComputedStyle(dom).right) < 0 ){
				dom.style.left = (dom.parentElement as HTMLDivElement).clientWidth- dom.offsetWidth+ "px"
			}
		})
