class VirtualScroller {
  ...
  toggleTopItems = (direction) => {
    ...
    if (direction === -1) {
      for (let index = this.#hiddenTopItemIndex; index > startIndex; index--) {
        this.contentBox.prepend(
          this.renderRow(
            this.renderItem(this.data[this.#hiddenTopItemIndex - 1]),
          ),
        );
        this.#hiddenTopItemIndex--;
      }
    ...
    this.contentBox.style.paddingTop =
      this.#hiddenTopItemIndex * this.rowHeight + "px";
  };
  toggleBottomItems = (direction) => {
    ...
    if (direction === 1) {
      for (
        let i = this.#hiddenTopItemIndex + this.contentBox.children.length;
        i <= lastExistingIndex;
        i++
      ) {
    ...
    let padd =
      (this.data.length -
        (this.#hiddenTopItemIndex + this.contentBox.children.length)) *
        this.rowHeight +
      "px";
