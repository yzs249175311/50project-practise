...
function next() {
    if (indexItem === items.length - 1) {
        ...
        var adjustTransformX = container.clientWidth * (items.length - 1) +
            parseFloat(getComputedStyle(container).transform.split(",")[4]);
        container.style.transform = "translateX(calc(100% + ".concat(adjustTransformX, "px))");
...
function prev() {
    if (indexItem === 0) {
        container.style.transitionDuration = "0s";
        //计算上一个动画还没有完成的偏移量
        var adjustTransformX = parseFloat(getComputedStyle(container).transform.split(",")[4]);
        container.style.transform = "translateX(calc(".concat(-100 * items.length, "% + ").concat(adjustTransformX, "px))");
        container.clientWidth;
