...
function currentPosition(): void {
	...
	let range_width: number = parseInt(getComputedStyle(range).getPropertyValue("width").match(/\d+/)[0])
	let show_width: number = parseInt(getComputedStyle(show).getPropertyValue("width").match(/\d+/)[0])
	let num: number = parseInt(range.value)
	let left = num * (range_width / 100) - show_width / 2 + scale(num, 0, 100, 13, -13)
