namespace insect_catch_game {
	let screens: NodeListOf<HTMLDivElement> = document.querySelectorAll(".screen")!
	...
	let time = 0
	let score = 0
	let game_over_div:HTMLDivElement = document.querySelector(".screen.game-container .over")!
	...
	insect_btns.forEach(item => {
		item.addEventListener("click", function (e) {
			let img: HTMLImageElement = (<HTMLLIElement>e.currentTarget).querySelector("img")!
			img_src = img.src
			start_game()
		})
	})
